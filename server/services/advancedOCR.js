          ...
          content: [
            {
              ...
              1. Extraia **todos os produtos** com nome completo, quantidade e valor unitário. O "amount" deve ser o preço unitário.
              ...
              3. Identifique automaticamente o tipo de pagamento com base em palavras como:
                - “CREDITO”, “CARTÃO DE CRÉDITO”, “Tao de Credito” → "type": "credit"
                - “DÉBITO”, “CARTÃO DE DÉBITO” → "type": "debit"
                - “DINHEIRO” → "type": "cash"
                - “PIX” → "type": "pix"
                - “CARTEIRA DIGITAL” → "type": "wallet"
                - “VALE”, “OUTRO” → "type": "other"
              4. Use **valores numéricos com duas casas decimais**.
              5. Se um dado não estiver presente, retorne `null`.
              6. Converta a data para o formato `DD/MM/YYYY`. Se o ano for de 2 dígitos (ex: 25), assuma 2025 se próximo à data atual (outubro 2025).
...

              {
                ...
                "metadata": {
                  ...
                  "total": 0.00,
                  "paymentMethod": {
                    "type": "credit|debit|cash|pix|wallet|other",
...
- Para linhas de itens como "7891193010012 BISN SEVEN BOYS 300G TRAD 1UN 5,49 5,49", extraia descrição "BISN SEVEN BOYS 300G TRAD", quantity 1, amount 5.49.
- Para linhas como "5056 COPO QUENCHER 420ML 1 PC X 49,90 49,90", extraia descrição "COPO QUENCHER 420ML", quantity 1, amount 49.90.
...
function extractMetadata(text) {
  ...
  // Extract date (DD/MM/YYYY or DD/MM/YY) and expand 2-digit year
  ...
  if (dateMatch) {
    ...
    if (parts.length === 3 && parts[2].length === 2) {
      ...
      // Adjust if future date
      const parsedDate = new Date(fullYear, parseInt(parts[1]) - 1, parseInt(parts[0]));
      if (parsedDate > today) {
        fullYear -= 100;
      }
      parts[2] = fullYear.toString();
      dateStr = parts.join('/');
    }
  ...
  if (lowerText.includes('cartao de credito') || lowerText.includes('credito') || lowerText.includes('tao de credito')) {
  ...
  } else if (lowerText.includes('carteira digital')) {
    metadata.paymentMethod.type = 'wallet';
    metadata.paymentMethod.details = 'Carteira Digital';
  }
...
function smartParseBrazilianReceipt(text) {
  ...
  const ignoreKeywords = [
    ...
    // Formas de pagamento
    'dinheiro', 'cartao', 'debito', 'credito', 'pix', 'especie', 'boleto', 'carteira digital',
  ...
  const patterns = [
    ...
    // Pattern 7: PRODUTO + QTD + UNIDADE + x + PREÇO + TOTAL
    // Ex: "COPO QUENCHER 1 UN X 49,90 49,90"
    /^(.+?)\s+[\d,]+\s+(?:PC|UN|KG|CX|LT|ML|G|PCT)\s+[xX*]\s*[\d,.]+\s+([\d,.]+)$/,

    // Pattern 8: CODE + DESCRIPTION + QTD UN + UNIT PRICE + TOTAL PRICE (for first receipt)
    // Ex: "7891193010012 BISN SEVEN BOYS 300G TRAD 1UN 5,49 5,49"
    /^(\d+)\s+(.+?)\s+(\d+)UN\s+([\d,]+)\s+([\d,]+)$/,

    // Pattern 9: CODE + DESCRIPTION + QTD UNIT X UNIT_PRICE TOTAL_PRICE (for second receipt)
    // Ex: "5056 COPO QUENCHER 420ML 1 PC X 49,90 49,90"
    /^(\d+)\s+(.+?)\s+(\d+)\s+(PC|UN|KG)\s+X?\s*([\d,]+)\s+([\d,]+)$/
  ];
  ...
  for (const line of lines) {
    ...
    for (let pIdx = 0; pIdx < patterns.length; pIdx++) {
      ...
      if (match) {
        ...
        let amountStr = match[match.length - 1].trim(); // Last capture is usually total price
        let quantity = 1; // Default

        // Specific handling for new patterns
        if (pIdx === 7 || pIdx === 8) { // Patterns 8 and 9 (0-based 7 and 8)
          description = match[2].trim();
          quantity = parseFloat(match[3].replace(',', '.'));
          // amountStr is total, but for unit price, use the one before if q>1
          const unitStr = match[4].trim();
          const totalStr = match[5].trim();
          amountStr = unitStr; // Use unit price as amount
        } else {
          // For other patterns, try to extract quantity
          const qtdMatch = line.match(/(\d+(?:,\d+)?)\s+(UN|PC|KG|CX|LT|ML|G|PCT)/i);
          if (qtdMatch) {
            quantity = parseFloat(qtdMatch[1].replace(',', '.'));
          }
        }
    ...
    if (!matched) {
      console.log(`[SmartParser] No pattern matched for line: "${line}"`);
    }
  }
...
async function extractReceiptData(imageBuffer) {
  ...
  try {
    ...
    // Ensure quantity is set for all items (default 1)
    ...
    for (const item of finalItems) {
      const key = `${item.description.toLowerCase()}_${item.amount.toFixed(2)}_${item.quantity}`;
      if (!seen.has(key)) {
        seen.add(key);
